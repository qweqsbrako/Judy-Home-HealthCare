...
public function nurseMobileDashboard(Request $request)
{
    ...
    $weekSchedules = Schedule::where('nurse_id', $user->id)
        ...
        ->map(function ($schedule) {
            ...
            // CRITICAL: Get actual duration from TimeTracking if schedule is completed
            $duration = $schedule->duration ?? '1h'; // Default to scheduled duration
            
            if ($schedule->status === 'completed' && $schedule->timeTracking) {
                // Use actual tracked time from time_trackings table
                $totalMinutes = $schedule->timeTracking->total_duration_minutes;
                if ($totalMinutes > 0) {
                    $hours = floor($totalMinutes / 60);
                    $minutes = $totalMinutes % 60;
                    
                    // Format: "2h 15m" or "45m" (no hours if less than 60 minutes)
                    if ($hours > 0) {
            ...
            return [
                ...
                'duration' => $duration, // ✅ NOW SHOWS ACTUAL TRACKED TIME FOR COMPLETED
                'status' => $schedule->status,
    ...
    $upcomingPatients = $upcomingSchedulesRaw
        ...
        ->map(function ($schedules) use ($now) {
            ...
            $patientSchedules = $schedules->map(function ($schedule) use ($now) {
                ...
                $scheduleDateTime = Carbon::parse($scheduleDate->format('Y-m-d') . ' ' . $scheduleTime->format('H:i:s'));
            ...
            return [
                ...
                'duration' => $firstSchedule->duration ?? '1h',
